...
func AddTypeUniversity(conn *sql.DB, body io.Reader) error {
	...
	err = typeUniversityDao.AddTypeUniversity(conn, m)
	if err != nil {
		return err
	}
	return nil
...
func GetTypeUniversityById(conn *sql.DB, id int64) (model.TypeUniversity, error) {
	m, err := typeUniversityDao.GetTypeUniversityById(conn, id)
	if err != nil {
...
func GetListTypeUniversity(conn *sql.DB) ([]model.TypeUniversity, error) {
	listModel, err := typeUniversityDao.GetListTypeUniversity(conn)
	if err != nil {
		return nil, err
	}
